...
# Define directories
SRC_DIR="$(pwd)"
RESULT_DIR="$SRC_DIR/macos_result"
...
LIB_FLAGS="-L/usr/local/lib -lfmt -lspdlog -pthread"

# Create result directories
mkdir -p "$RESULT_DIR" "$BUILD_DIR" "$OUTPUT_DIR"

# 1. Compile EdgeClient.cpp (for static & shared libraries)
g++ -std=c++20 -Wall -Wextra -O2 -fPIC $INCLUDE_FLAGS -c "$SRC_FILES" -o "$OBJ_FILE"
...
g++ -std=c++20 -Wall -Wextra -O2 $INCLUDE_FLAGS -c "$SAMPLE_SRC" -o "$SAMPLE_OBJ"
